<script>
    $(function() {
        const accessUpdate = Boolean({{ check_user_access('store', 'update') }})
        const accessDelete = Boolean({{ check_user_access('store', 'delete') }})

        const table = $('#table').DataTable({
            ajax: {
                url: "{{ route('store.datatables') }}",
                method: 'GET',
            },
            pageLength: 10,
            serverSide: true,
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: (m,t,data) => {
                        return `
                        <img src="{{ url('uploads') }}/${data.logo}" alt="${data.name}" width="50" height="50">
                        `
                    }
                },
                {
        ...
        $('#create-btn').on('click', function() {
            $('#add-modal').find('.modal-body').load('{{ route('store.create') }}')
        })

        // Show Edit Modal
        $('#app').on('click', '.edit-btn', function() {
            const id = $(this).attr('data-id')
            const url = '{{ route('store.edit', '-id-') }}'.replace('-id-', id)
            ...
            $.ajax({
                ...
                error: (error) => {
                    ...
                    if ( error.status === 400 ) {
                        ...
                        $('#area-error').text(data?.area)
                        $('#address-error').text(data?.address)
                        $('#logo-error').text(data?.logo)
                        $('#whatsapp-error').text(data?.whatsapp)
                        $('#bank-error').text(data?.bank)
                        $('#account_number-error').text(data?.account_number)
                        $('#account_name-error').text(data?.account_name)
        ...
        $('#edit-form').on('submit', function(e) {
            ...
            $.ajax({
                url: '{{ route('store.update', '-id-') }}'.replace('-id-', id),
                method: 'POST',
                contentType: false,
                processData: false,
                data: formData,
        ...
        $('#app').on('click', '.delete-btn', function() {
            ...
            $.ajax({
                url: '{{ route('store.destroy', '-id-') }}'.replace('-id-', id),
                method: 'POST',
                data: {
                     _method: 'DELETE',
