<script>
    $(function() {
        const accessUpdate = Boolean({{ check_user_access('user-access-item', 'update') }})
        const accessDelete = Boolean({{ check_user_access('user-access-item', 'delete') }})

        const table = $('#table').DataTable({
            ajax: {
                url: "{{ route('user-access-item.datatables') }}",
                method: 'GET',
            },
            pageLength: 10,
            serverSide: true,
                ...
                },
                {
                    ...
                    render: (m, t, data) => {
                        ...
                        <div class="d-flex justify-content-center align-items-center gap-1">
                            ${accessUpdate ? `
                            <button data-id="${data.id}" title="Edit Data" class="edit-btn btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#edit-modal">
        ...
        $('#create-btn').on('click', function() {
            $('#add-modal').find('.modal-body').load('{{ route('user-access-item.create') }}')
        })
        ...
        $('#create-form').on('submit', function(e) {
            ...
            formData.set('access_menu', formData.get('access_menu') === 'on' ? 1 : 0)
            formData.set('access_read', formData.get('access_read') === 'on' ? 1 : 0)
            formData.set('access_create', formData.get('access_create') === 'on' ? 1 : 0)
            formData.set('access_update', formData.get('access_update') === 'on' ? 1 : 0)
            formData.set('access_delete', formData.get('access_delete') === 'on' ? 1 : 0)
            formData.set('access_member', formData.get('access_member') === 'on' ? 1 : 0)
            formData.set('access_approve', formData.get('access_approve') === 'on' ? 1 : 0)

            $.ajax({
                url: '{{ route('user-access-item.store') }}',
                method: 'POST',
                contentType: false,
                processData: false,
                data: formData,
                ...
                error: (error) => {
                    ...
                    if ( error.status === 400 ) {
                        ...
                        $('#access_name-error').text(data?.access_name)
                        $('#access_link-error').text(data?.access_link)
                ...
                beforeSend: () => {
                    ...
                    /* Reset Error */
                    $('#access_name-error').text('')
                    $('#access_link-error').text('')
                },
                complete: () => {
