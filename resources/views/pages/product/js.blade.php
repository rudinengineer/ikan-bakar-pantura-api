<script>
    $(function() {
        const accessUpdate = Boolean({{ check_user_access('product', 'update') }})
        const accessDelete = Boolean({{ check_user_access('product', 'delete') }})

        const table = $('#table').DataTable({
            ajax: {
                url: "{{ route('product.datatables') }}",
                method: 'GET',
            },
            pageLength: 10,
            serverSide: true,
            ...
            columns: [{
                ...
                @endif
                {
                    ...
                    render: (m,t,data) => {
                        return `
                        <img src="{{ url('uploads') }}/${data.image}" alt="${data.name}" width="50" height="50">
                        `
                    }
                },
                {
                ...
                },
                {
                    data: 'price',
                    render: (m,t,data) => {
                        return `Rp ${new Intl.NumberFormat('id-ID', {currency:'IDR'}).format(data.price)}`
                    }
                },
                {
                    data: 'id',
        ...
        $('#create-btn').on('click', function() {
            $('#add-modal').find('.modal-body').load('{{ route('product.create') }}')
        })

        // Show Edit Modal
        $('#app').on('click', '.edit-btn', function() {
            const id = $(this).attr('data-id')
            const url = '{{ route('product.edit', '-id-') }}'.replace('-id-', id)
            ...
            $.ajax({
                ...
                error: (error) => {
                    ...
                    if ( error.status === 400 ) {
                        ...
                        $('#price-edit-error').text(data?.price)
                        $('#image-edit-error').text(data?.image)
