<script>
    $(function() {
        const accessUpdate = Boolean({{ check_user_access('user-management', 'update') }})
        const accessDelete = Boolean({{ check_user_access('user-management', 'delete') }})

        const table = $('#table').DataTable({
            ajax: {
                url: "{{ route('user-management.datatables') }}",
                method: 'GET'
            },
            pageLength: 10,
            serverSide: true,
            ...
            columns: [{
                ...
                },
                {
                    data: 'phone',
                    render: (m, t, data) => {
                        return data.phone_prefix + data.phone
                    }
                },
                {
                    data: 'role.name',
        ...
        $('#create-btn').on('click', function() {
            $('#add-modal').find('.modal-body').load('{{ route('user-management.create') }}')
        })
            ...
            $.ajax({
                ...
                error: (error) => {
                    ...
                    if ( error.status === 400 ) {
                        ...
                        $('#phone-error').text(data?.phone)
                        $('#role_id-error').text(data?.role_id)
            ...
            $.ajax({
                ...
                error: (error) => {
                    ...
                    if ( error.status === 400 ) {
                        ...
                        $('#username-edit-error').text(data?.username)
                        $('#email-edit-error').text(data?.email)
                        $('#phone-edit-error').text(data?.phone)
                        $('#role_id-edit-error').text(data?.role_id)
                        $('#password-edit-error').text(data?.password)
                        $('#is_active-edit-error').text(data?.is_active)
                ...
                beforeSend: () => {
                    ...
                    $('#username-edit-error').text('')
                    $('#email-edit-error').text('')
                    $('#phone-edit-error').text('')
                    $('#role_id-edit-error').text('')
                    $('#password-edit-error').text('')
                    $('#is_active-edit-error').text('')
                },
                complete: () => {
